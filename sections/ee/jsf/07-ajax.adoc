...
Listing Example 5 with Ajax via JavaScript shows again the example with the two selection lists from the last section. 
...
Listing callback function for Ajax events shows a function that outputs a message for each event. 
...
If you need a callback function for a specific Ajax request, you can with the attribute onevent of `f:ajax` or when calling `jsf.ajax.request()` as an option with the key onevent register.
...
addon error() can also be called multiple times in succession to register more than one function. 
...
This is the lifecycle, as in Figure lifecycle with Ajax to recognize, divided into the two logical areas of execution and rendering.
...
The example in Listing Example for delay of `f:ajax` shows an input component that triggers an Ajax request each time the user presses the button. 
...
Why this can be necessary is shown in the example in Listing 

Example of resetValues. The code looks perfect at first glance. 
...
The reason for this is quickly found. During the request with the invalid value for v2, JSF set the local value for both components in the process validation phase. 
...
In the render phase of the partial lifecycle, we need to re-render two components in this case. On the one hand the panel group with the content of the component and on the other hand also the button itself, because the icon changes depending on the fold-in state. 
...
But that's not all. We go one step further and also move part of the content added to the composite component to Ajax. 
In the viewCustomer.xhtml view, the list of addresses of a customer is contained in a composite component of the type dataTable (see section Section: The component mc: dataTable ). The table is in turn into one embedded collapsiblePanel component. 
...
Listing composite component ajaxStatus shows the declaration.
...
Listing composite component ajaxPoll shows the declaration.
...
If ajaxStatus is used at the same time, the status message is displayed with every update - a behavior that is not always appropriate. But even there is a very simple solution. 
...
Figure MyGourmet 14 with Ajax status shows a screenshot of view `showCustomer.xhtml` enabled Ajax status message.
...
Listing The component ajaxStatus in use shows the updated header area.
